...
class PieChartView: UIView {
    ...
    var segmentLabelFont = UIFont.systemFont(ofSize: 20) {
        didSet {
            textAttributes[kCTFontAttributeName as String]  =  segmentLabelFont
        }
    }
    ...
    private lazy var textAttributes : [String : Any] = {
        return [kCTParagraphStyleAttributeName : self.paragraphStyle, kCTFontAttributeName : self.segmentLabelFont]
        }() as [String : Any]
    ...
    override func draw(_ rect: CGRect) {
        ...
        for segment in segments { // loop through the values array
            ...
            if showSegmentLabels { // do text rendering
                ...
                let textToRender = showSegmentValueInLabel ? "\(segment.name) (\(segment.value))" : segment.name
                ...
                textAttributes[kCTForegroundColorAttributeName as String] = (averageRGB > 0.7) ? UIColor.black : UIColor.white
                
                // the bounds that the text will occupy
                var renderRect = CGRect(origin: .zero, size: CGSize.init(width: 30, height: 30))
                
                // center the origin of the rect
                renderRect.origin = CGPoint(x: segmentCenter.x - renderRect.size.width * 0.3, y: segmentCenter.y - renderRect.size.height * 0.5)
                
                // draw text in the rect, with the given attributes
                textToRender.draw(in: renderRect, withAttributes: (textAttributes["kCTFontAttributeName"] as? [NSAttributedStringKey : Any]))
            }
